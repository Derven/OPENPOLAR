...
/* Commenting this out for now until we figure out what to do with shuttle smooth walls, if anything.
   As they are now, they sort of work screwy and may need further coding. Or just be scrapped.*/
	...
	else if (istype(src,/obj/structure/snow))
		var/obj/structure/snow/spiece = src
		spiece.icon_state = "[spiece.mineral][junction]"
...
/atom/proc/relativewall_neighbours()
	...
	for(var/obj/structure/falsewall/W in range(src,1))
		W.relativewall()
		W.update_icon()//Refreshes the snow to make sure the icons don't desync
	for(var/obj/structure/snow/S in range(src,1))
		S.relativewall()
		S.update_icon()//Refreshes the wall to make sure the icons don't desync
	...
	for(var/obj/structure/falsewall/W in orange(src,1))
		if(abs(src.x-W.x)-abs(src.y-W.y)) //doesn't count diagonal walls
			if(src.mineral == W.mineral)
				junction |= get_dir(src,W)
	for(var/obj/structure/snow/S in orange(src,1))
		if(abs(src.x-S.x)-abs(src.y-S.y)) //doesn't count diagonal walls
			if(src.mineral == S.mineral)
				junction |= get_dir(src,S)
	for(var/obj/structure/falserwall/W in orange(src,1))
		if(abs(src.x-W.x)-abs(src.y-W.y)) //doesn't count diagonal walls
			if(src.mineral == W.mineral)
				junction |= get_dir(src,W)
