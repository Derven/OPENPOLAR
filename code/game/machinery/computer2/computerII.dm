...
/obj/machinery/computer2
	...
	var/installed_prog = null //Добавляет файлы на пекарню. Изначально их нет.
...
/obj/item/weapon/disk/data/computer2test
	...
	New()
		..()
		src.root.add_file( new /datum/computer/file/computer_program/arcade(src)) // arcade.dm
		src.root.add_file( new /datum/computer/file/computer_program/med_data(src)) // med_rec.dm
		src.root.add_file( new /datum/computer/file/computer_program/airlock_control(src)) // airlock_control.dm
		src.root.add_file( new /datum/computer/file/computer_program/messenger(src)) // messenger.dm
		src.root.add_file( new /datum/computer/file/computer_program/progman(src)) // hz
		src.root.add_file( new /datum/computer/file/computer_program/sec_data(src)) // sec_rec.dm

/obj/machinery/computer2/medical
	name = "medical computer"
	icon_state = "medcomp"
	setup_drive_size = 64.0
	setup_has_radio = 1
...
/obj/machinery/computer2/security
...
/obj/machinery/computer2/New()
	...
	spawn(5)
		...
		if(ispath(src.installed_prog))
			new src.installed_prog(src)
...
/obj/machinery/computer2/attack_hand(mob/user as mob)
	...
	if((src.active_program) && (src.active_program.master == src) && (src.active_program.holder in src))
		...
		dat = "<TT><b>Lindows Pista</b><br><br>"

		dat += "Текущий ID: <a href='?src=\ref[src];id=auth'>[src.authid ? "[src.authid.name]" : "----------"]</a><br>"
		dat += "Дополнительный ID: <a href='?src=\ref[src];id=aux'>[src.auxid ? "[src.auxid.name]" : "----------"]</a><br><br>"
