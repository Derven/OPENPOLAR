...
/datum/computer/file/computer_program/sec_data/return_text()
	...
	if (src.temp)
		dat = text("<TT>[src.temp]</TT><BR><BR><A href='?src=\ref[src];choice=Clear Screen'>Clear Screen</A>")
	else
		//dat = text("Confirm Identity: <A href='?src=\ref[];choice=Confirm Identity'>[]</A><HR>", master, (src.master.authid ? text("[]", src.master.authid.name) : "----------"))
		if (src.authenticated)
			switch(src.screen)
				if(1.0)
					dat += {"<p style='text-align:center;'>"}
					dat += text("<A href='?src=\ref[src];choice=Search Records'>Search Records</A><BR>")
					dat += text("<A href='?src=\ref[src];choice=New Record (General)'>New Record</A><BR>")
					dat += {"
</p>
...
<th>Criminal Status</th>
				...
				if(3.0)
					...
					if ((istype(active1, /datum/data/record) && data_core.general.Find(active1)))
						...
						mob << browse_rsc(front, "front.png")
						mob << browse_rsc(side, "side.png")*/
						...
						Mental Status: [active1.fields["m_stat"]]<BR></td></table>")
						/*<td align = center valign = top>Photo:<br><img src=front.png height=80 width=80 border=4>	\
						<img src=side.png height=80 width=80 border=4></td></tr>)*/
					else
...
/datum/computer/file/computer_program/sec_data/Topic(href, href_list)
	...
	if (!( data_core.security.Find(active2) ))
		active2 = null
	if ((usr.contents.Find(src) || (in_range(src, usr) && istype(src, /turf))) || (istype(usr, /mob/living/silicon)))
		usr.set_machine(src)
...
/*			if("Confirm Identity")
				if (src.master.authid)
					if(istype(usr,/mob/living/carbon/human) && !usr.get_active_hand())
						usr.put_in_hands(src.master.authid)
					else
						src.master.authid.loc = get_turf(src)
			...
			if("Log In")
				if (istype(usr, /mob/living/silicon/ai))
					...
					active1 = null
					active2 = null
					if(check_access(src.master.authid))
						authenticated = src.master.authid.registered_name
						rank = src.master.authid.assignment
						screen = 1
//RECORD FUNCTIONS
			if("Search Records")
