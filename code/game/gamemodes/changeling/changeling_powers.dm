...
/mob/proc/make_changeling_mudblood()
	if(!mind)				return
	if(!mind.changeling)	mind.changeling = new /datum/changeling(gender)
	src.remove_changeling_powers()
	src.verbs += /mob/proc/changeling_mudblood_fakedeath
...
/mob/proc/changeling_absorb_dna()
	...
	if(prob(changeling.absorbedcount*3 - 3))
		T.make_changeling_mudblood()
		T << "\red <FONT size = 3>You are now a mudblood-changeling!</FONT>"
	return 1
...
/* /mob/proc/changeling_unknown_form()
	set category = "Changeling"
	set name = "Unknown form (1)"
	...
	var/mob/living/carbon/thething/O = new /mob/living/carbon/thething(src)
	O.dna = C.dna
	C.dna = null
	...
	O.loc = C.loc
	O.name = "the thing ([copytext(md5(C.real_name), 2, 6)])"
	...
	O.make_changeling(1)
	O.verbs += /mob/proc/changeling_unknown_form
...
/mob/proc/changeling_mudblood_fakedeath()
	set category = "Changeling"
	set name = "Mudblood's stasis (20)"
