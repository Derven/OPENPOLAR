	...
	if (istype(W, /obj/item/weapon/fishing_rod))
		if (hole != 0)
			...
			var/obj/haul = pick(25*rod.carp_prob_modifier; /mob/living/simple_animal/hostile/carp,
								1; /obj/item/clothing/gloves/fluff/chal_appara_1,
								1; /obj/item/device/flashlight/fluff/thejesster14_1,
			...
			if (!haul)
				...
				user << "<span>\blue You have cought a [haul]! Nice one.</span>"
			else
				if (istype(haul, /mob/living/simple_animal/hostile/carp))
					user << "<span>\red [user] cancels fish: Interrupted by carp!</span>"
				else
