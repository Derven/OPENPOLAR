	...
	spawn(2) //So it properly updates when deleting
		...
		if(istype(src,/obj/structure/table/reinforced))
			switch(table_type)
				...
				if(1)
					icon_state = "reinf1tileendtable"
				if(2)
					icon_state = "reinf1tilethick"
				if(3)
					icon_state = "reinftabledir"
				if(4)
					icon_state = "reinfmiddle"
				if(5)
					icon_state = "reinftabledir2"
				if(6)
					icon_state = "reinftabledir3"
