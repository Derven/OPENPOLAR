	...
	while(L.len)
		...
		if(down && (istype(T, /turf/space) || istype(T, /turf/simulated/floor/open)))
			...
			if(below)
				...
				T.overlays += image('icons/turf/floors.dmi', icon_state = "black_open", layer = TURF_LAYER+1)
				T.z_overlays += image('icons/turf/floors.dmi', icon_state = "black_open", layer = TURF_LAYER+1)
