/mob/living/carbon/thething/emote(var/act,var/m_type=1,var/message = null)
	...
	switch(act)
		...
		if("sign")
			if (!src.restrained())
				message = text("<B>The thing</B> signs[].", (text2num(param) ? text(" the number []", text2num(param)) : null))
				m_type = 1
		if("scratch")
			if (!src.restrained())
