...
/mob/living/carbon/human
	...
	proc/handle_breath(datum/gas_mixture/breath)
		...
		if(CO2_pp > safe_co2_max && !CO2_filter)
...
//				log_debug("Hot. Difference = [body_temperature_difference]. Recovering [recovery_amt]")
	...
	proc/get_cold_protection(temperature)
		...
		if(COLD_RESISTANCE in mutations || northern_man == 1)
	...
	proc/handle_regular_hud_updates()
		...
		if( stat == DEAD )
			...
			if(glasses)
				...
				else if(istype(glasses, /obj/item/clothing/glasses/night) || night == 1)
					see_in_dark = 5
					if(!druggy)
						see_invisible = SEE_INVISIBLE_MINIMUM
...
//				else									rest.icon_state = "rest0"
			...
			if(blind)
				if(blinded)		blind.alpha = 255//blind.layer = 18
				else			blind.alpha = 0//blind.layer = 0
