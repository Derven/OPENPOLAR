...
/datum/admins/proc/quick_create_object(var/mob/user)
	...
	if (!quick_create_object_html)
		...
		quick_create_object_html = replace_text(quick_create_object_html, "null /* object types */", "\"[objectjs]\"")

	user << browse(replace_text(quick_create_object_html, "/* ref src */", "\ref[src]"), "window=quick_create_object;size=425x475")